<?php
 ...
 $sql='UPDATE students_registration_data SET firstname=:firstname, surname=:surname, other_name=:other_name, reg_no=:reg_no, department=:department,  library=:library WHERE id=:id';
 $statement = $connection->prepare($sql);
 if($statement->execute([':firstname'=>$firstname, ':surname'=>$surname, ':other_name'=>$other_name, ':reg_no'=>$reg_no, ':department'=>$department, ':library'=>$library ." ".  $reason,':id'=>$id])){
...
<div class="col-md-4 offset-md-4 rounded"></div>
    ...
    <div class="form-group">
     ...
     <div class="form-group">
     <label>Library</label>
        <select id="Librarian" class="form-control" name="library" autofocus required>
