<?php
 ...
 $sql='UPDATE students_registration_data SET firstname=:firstname, surname=:surname, other_name=:other_name, reg_no=:reg_no, department=:department,  dean=:dean WHERE id=:id';
 $statement = $connection->prepare($sql);
 if($statement->execute([':firstname'=>$firstname, ':surname'=>$surname, ':other_name'=>$other_name, ':reg_no'=>$reg_no, ':department'=>$department, ':dean'=>$dean ." ".  $reason,':id'=>$id])){
 
 header("location: deanclearanceform.php");
...
<div class="col-md-4 offset-md-4 rounded"></div>
    ...
    <div class="form-group">
     ...
     <label>Dean of Students</label>
        <select id="DeanStudent" class="form-control" name="dean" autofocus>
...
$('#DeanStudent').change(function(){   
if($('#DeanStudent').val() === 'uncleared')   
   {   
   $('#Uncleared').show();    
   }   
else 
